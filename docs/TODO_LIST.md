...
- [ ] **README.md 가이드 업데이트**: 실제 리허설에 사용된 정확한 셸 명령어를 README에 추가하여 다른 에이전트가 쉽게 똑같이 실행할 수 있도록 함.
- [ ] **Pygame 테스트 러너 프리셋 보완**: 실제 윈도우 창이 없는 headless 환경(CI/CD 등)에서도 스모크 테스트 및 렌더링이 문제없이 작동하도록 캡처 기능 안정화.
