...
가장 중요한 Task입니다. 추가 기능 개발보다 **AISurvivalMini(Pygame 데모 프로젝트) 및 Unity 데모 프로젝트를 활용하여 실제 전체 루프가 매끄럽게 돌아가는지 리허설**해야 합니다.
...
- [x] **Unity 데모 프로젝트 워크스페이스 연동 및 첫 번째 태스크 성공** ✅ (2026-06-17 완료):
  - `unity-game.git` 베어 저장소 및 워크스페이스 생성, 데이터베이스 연동.
  - 첫 번째 워커 태스크(초기 프로젝트 스켈레톤 폴더 구조 및 .gitignore 구성)를 임대, 커밋, 푸시 후 오너 승인 머지까지 성공적으로 완료.

### 🔐 [우선순위 2] 보안 및 대용량 파일 전송
- [x] **대용량 파일 스트리밍 업로드 구현** ✅ (2026-06-17 완료): 현재 100MiB 업로드 제한이 설정되어 있으나, 리퀘스트 바디 전체를 메모리에 올리는 방식에서 request.stream()을 활용한 스트리밍 방식으로 개선 완료.

### 🤖 [우선순위 3] Discord 봇 및 승인(Approval) 시스템 연동
- [ ] **실제 Discord 서버 연동 테스트**: 현재 구현된 Discord Gateway를 실제 테스트용 Discord 서버에 띄워서 정상 작동 확인.
- [x] **자연어 기반 승인/의사결정 플로우 연결** ✅ (2026-06-17 완료): #approval-inbox 성격의 디스코드 채널에서 사용자가 보낸 자연어(예: "좋아 진행해", "거절해")를 감지하고 의도를 판단하여, 서버 API(/approvals/{approval_id}/decision)를 통해 결재를 승인/반려하는 흐름 구현 완료.
