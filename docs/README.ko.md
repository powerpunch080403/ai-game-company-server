...
## 장기 제품 비전 (Long-Term Product Vision)

AI Project Control Server의 장기 비전은 Personal, Team, Enterprise의 점진적 배포 단계를 거쳐 인간 기여자들과 AI 워커들을 조율하고, 작업 대여(Lease), 범위(Scope), 락(Lock)을 프로젝트 실행의 공통 문법으로 관리하는 제어 평면을 제공하는 것입니다.

상세한 제품 정의, 역량 기반 워커 스케줄링, 워크플로우 정책, 프런트엔드/데이터베이스 진화 계획은 다음 문서를 참조하십시오:
* **[제품 비전 및 장기 개발 방향 (Product Vision and Long-Term Direction)](../docs/PRODUCT_VISION.md)**
...
- **v1.5 오너 태스크 계획 및 Discord 스레드 워크플로우**: 읽기 전용 Project Search, Task Plan Search, 스레드 선택 생성 기능이 포함된 Tasks From Plan, task_thread_reference를 통한 외부 스레드 연결, 그리고 최선 노력(best-effort) 방식의 워커 보고 Discord 완료 요약 포스팅.
...
- [Product Vision (제품 비전)](../docs/PRODUCT_VISION.md)
...
이 스모크 테스트에 사용할 재사용 가능한 Owner 에이전트 프롬프트는 [`docs/V1_SMOKE_TEST.md`](V1_SMOKE_TEST.md)에 저장되어 있습니다.
...
### 주요 워크플로우 단계
서버는 계획 검색부터 태스크 생성, 워커 실행, 보고서 포스팅까지 다음 순서로 조정합니다:
1. **프로젝트 검색 (Project Search)**: ripgrep을 사용하여 로컬 작업 공간을 읽기 전용으로 검색해 관련 파일을 찾습니다.
2. **태스크 계획 검색 (Task Plan Search)**: 적절한 범위(`read_scope`/`write_scope`)와 프롬프트 컨텍스트 초안을 제안합니다.
3. **계획 기반 태스크 생성 (Tasks From Plan)**: 제안된 범위와 컨텍스트를 사용하여 실제 태스크를 생성합니다.
4. **Discord 스레드 생성 (선택 사항)**: `create_thread=true`인 경우 전용 Discord 태스크 스레드를 생성합니다.
5. **스레드 레퍼런스 저장**: 태스크와 외부 Discord 스레드를 연결하는 `task_thread_reference`를 데이터베이스에 보존합니다.
6. **워커 대여 (Worker Lease)**: 태스크를 배정하여 워커 브랜치를 생성하고, `base_commit`을 기록하며, `task_locks`를 걸어 파일 충돌을 방지합니다.
7. **워커 보고 (Worker Report)**: 워커가 변경 파일을 보고하면 `write_scope` 및 `forbidden_scope`와 비교해 검증합니다.
8. **Discord 보고 포스팅 (최선 노력)**: 완료 보고서 요약을 연결된 Discord 스레드에 최선 노력(best-effort) 방식으로 포스팅합니다.
9. **병합 후보 큐 등록**: 보고서 검증 결과가 성공이면 `merge_candidates`에 큐 상태로 후보를 자동 등록합니다.
10. **오너 승인/반려**: 오너가 후보를 검증해 승인 또는 반려합니다.
11. **병합 실행**: dry-run 검증 완료 후 최종 병합을 처리합니다.

### 엔드포인트 요약
* `POST /projects/{project_id}/search`: 작업 공간 읽기 전용 ripgrep 검색.
* `POST /projects/{project_id}/task-plan/search`: 범위 제안 및 프롬프트 초안을 만드는 계획 검색.
* `POST /projects/{project_id}/tasks/from-plan`: 계획 검색 제안을 토대로 태스크를 생성하고 Discord 스레드를 선택적으로 개설.
* `PUT /tasks/{task_id}/thread-reference`: 태스크에 외부 스레드 레퍼런스를 수동 연결.
* `GET /tasks/{task_id}/thread-reference`: 태스크의 스레드 레퍼런스 정보 조회.
* `GET /projects/{project_id}/thread-references`: 프로젝트 내 모든 스레드 레퍼런스 목록 조회.
* `POST /workers/{worker_id}/tasks/{task_id}/report`: 워커의 결과 완료 보고 (Discord 결과 요약 자동 포스팅 트리거).

### 계획 기반 태스크 생성 (`tasks/from-plan`) 세부 사양
* 요청 페이로드에 `confirm=true`가 필수적입니다.
* `create_thread=false`인 경우 스레드를 생성하지 않고 기본 태스크 생성만 동작합니다.
* `create_thread=true`인 경우 Discord 스레드를 개설합니다 (정상적인 Discord 설정 필요).
* `thread_channel_id`를 명시하여 기본 채널을 덮어쓸 수 있습니다.
* 수동 `thread_reference`와 `create_thread=true` 옵션은 동시에 함께 사용해서는 안 됩니다.
* 태스크 생성 중 Discord 스레드 개설에 실패하면 즉시 `503 Service Unavailable` 오류가 명시적으로 반환됩니다.
* Discord 결과 포스팅은 최선 노력(best-effort) 방식으로 작동하며, 전송 실패가 워커 완료 보고 자체를 실패로 만들지 않습니다.

### 환경 설정 값
`.env.example`을 `.env`로 복사해 설정합니다. **실제 `.env` 파일은 절대 Git에 커밋하지 마십시오.**
* `DISCORD_BOT_TOKEN`: 스레드 생성 및 완료 요약 포스팅에 사용할 Discord 봇 토큰.
* `GAME_COMPANY_DISCORD_TASK_CHANNEL_ID`: 스레드가 개설될 기본 Discord 채널 ID.

### 안전 및 격리 보장
* **검색 제외**: 프로젝트 검색 시 `.env`, `.git`, `node_modules`, 가상 환경 및 캐시 폴더는 기본적으로 제외되어 보안을 유지합니다.
* **범위 검증**: `write_scope`와 `forbidden_scope` 정책으로 쓰기를 제약합니다. 위반 시 `scope_violation` 상태로 차단됩니다.
* **락 제어**: `task_locks`를 통해 중첩된 파일 쓰기 영역 간의 동시 수정 충돌을 원천 차단합니다.
* **작업 신선도**: `base_commit` 추적으로 기준 브랜치가 전진했는지 감지하여 `needs_rebase`를 부여하고 머지를 제약합니다.
* **장애 격리**: Discord API 전송 오류는 격리되어 코어 태스크 처리나 워커 완료 보고 동작을 방해하지 않습니다.
* **메모리 인덱스화**: 상세 대화나 상세 로그는 외부 Discord 스레드에 두고, 서버 DB에는 가벼운 요약 및 레퍼런스 인덱스만 기록해 리소스를 절약합니다.

### 현재 비목표 (Non-Goals)
* 자동 코드 품질 검사원(Project Reviewer) 에이전트 미지원.
* Discord 스레드 대화 이력 자동 가져오기(fetching) 미지원.
* 원본 Discord 스레드의 자동 LLM 요약 미지원.
* 벡터 메모리 검색 미지원 (SQLite 기본 검색 사용).
* 오너의 수동 승인 없는 자동 최종 병합 미지원.
