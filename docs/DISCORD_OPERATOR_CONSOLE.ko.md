...
- Discord guild/channel/thread mapping schema/API는 구현되어 있다.
- `app.discord_bot`은 실제 Discord Gateway 접속 전 단계의 routing skeleton이다.
- `scripts/run_discord_bot.*`는 mapping context와 message content를 받아 dry-run으로
  어떤 bot action이 필요한지 출력한다. Offline dry-run은 JSON 없이도
  `--project-id`, `--conversation-kind`, `--thread-role`로 실행할 수 있다.

아직 코드 구현 전 설계/구현 항목:

- `discord.py` 또는 대체 Discord Gateway adapter.
...
## Current Bot Skeleton Contract

Bot skeleton은 다음 순서로 동작한다:

1. Discord message의 `guild_id`, `channel_id`, `thread_id`를 받는다.
2. 서버 `/discord/mappings`에서 active mapping을 조회한다.
3. thread mapping이 있으면 thread mapping을 우선 사용한다.
4. thread mapping이 없으면 channel-level mapping으로 fallback한다.
5. mapping의 `conversation_kind`와 `thread_role`에 따라 action을 만든다.
...
ai_internal_observation
artifact_observation
test_runner_observation
record_only
```

이 skeleton은 아직 Discord에 로그인하지 않는다. 실제 Discord 접속은
`DISCORD_BOT_TOKEN` and Gateway adapter를 붙이는 다음 단계에서 진행한다.
